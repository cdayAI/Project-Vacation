...
**Wave 9 — the document generator is reachable, and owner-facing goes through consent. T-12.**

- **T-12 — the documents subsystem was built and inert.** `buildPlatform` composes the `DocumentStore`, the `TemplateRegistry`, and the `DocumentGenerator` — handed the platform's one already-composed `contactGate`, not a second — and registers `DOCUMENT_ACTIONS`; `pv documents` (template register/approve/list/show, propose, generate) reaches the real services. Driven against Postgres by two independent parties and by hand: a draft refuses generation; self-approval refuses; a different reviewer approves; an internal document generates with full provenance (content-derived data digest reproducible across runs); a consumer template needs two distinct approvers; an owner-facing generation with no consent is refused at the contact gate (`contact.no_consent`), and with consent + a proposal-digest-bound approval granted by a different person is produced, recording an approver distinct from the producer. Every refusal leaves zero document rows; the audit chain stays intact. Residual: no HTTP route/console surface, artifact preview still unwired, text/HTML only (PDF/DOCX a declared seam, L7) — see T-12. Platform 83 files / 2,052 tests, tsc/lint clean, no existing test weakened.

**Wave 10 — MVW's systems of record are reachable through one governed integration path. T-13.**

- **T-13 — the integrations layer was built and inert.** `buildPlatform` now composes the whole governed path and exposes it on `Platform`: the contract- and association-records ports (seeded fakes as the shipped dev path — every descriptor carries `shapeConfirmedWithMvw:false`, because no adapter has yet seen MVW's real systems); the `EgressClient` whose host allowlist is wired from `PV_EGRESS_ALLOWLIST`, so the allowlist finally bounds a reachable client, recording a run step before every call and re-checking containment between attempts; the `DegradationHandler` over the Pg-vs-memory integration queue; and env-backed, host-scoped, sealed credentials that fail closed. `pv integrations` (list, contract show, association show, call) reads the systems of record and drives the egress path. Driven against Postgres: a seeded contract (jurisdiction FL) and association (budget + reserve) read through the composed ports; a call to an **unallowlisted** host is refused before anything leaves the process (`integration.host_not_allowlisted`, no step recorded); a call whose credential is unset is refused (`integration.credential_missing`); a genuine failure under a `queue` policy queues (row in `integration_queued_call`, step recorded `failed`) and under `refuse` refuses; a `DeniedError` is never degraded; the credential never appears in a step, a log, or the audit chain. **S10 investigated, no change made:** the OIDC discovery/JWKS path is self-bounded to the issuer host by construction (`redirect:manual` plus a host check that refuses any endpoint off the issuer), which is tighter than an allowlist — routing it through the allowlist would loosen it and is declined honestly. Residual: no real HTTP adapter implements the ports (fakes are the dev path, L4); egress governance is transport-level; CLI only — no HTTP route/console surface. Platform 84 files / 2,074 tests, tsc/lint clean, no AI attribution, no existing test weakened.

**Wave 11 — the console reaches every CLI-wired subsystem: the eight missing HTTP routes. T-14.**

- **T-14 — the console called eight endpoints the server never served.** All eight are now served, each a thin handler that re-checks authorization at the chokepoint and calls a per-surface view mapper (`api/role-registry.ts`, `api/workflow-instance.ts`, `api/improvement-clusters.ts`, `api/improvement-proposals.ts`, `api/discovery-backlog.ts`, `api/executive.ts`): `GET /api/roles` and `/api/roles/:id/versions` (role registry + version history, newest first, 404 on an absent role); `GET /api/workflows/:id` (`engine.describeInstance`, 404 when absent); `GET /api/improvements/clusters` (real, recomputed on demand via `clusterObservations` over the now-exposed observation store); `GET /api/improvements/proposals` and `/:id` (the `ProposalStore` read side, honestly empty because the propose stage stays unwired, 404 on a miss); `GET /api/discovery/candidates` (empty while the feature ships disabled, `draftOnly` always true); and `GET /api/executive` — the business-value board that **never credits the platform for a movement it did not cause**: six MVW figures (contract sales, VPG, adjusted EBITDA, diluted EPS, financing margin, interval members) transcribed from MVW's own Q2 2026 8-K, each tile's `sourceNote` reading "MVW reported … Not attributable to this platform"; three platform tiles counted from the operating record marked "Measured by this platform"; `humanHoursSaved` omitted with a caveat because it is not yet measured. The console-contract ratchet's `KNOWN_UNIMPLEMENTED` is now `[]`, and both halves stay green. Driven against Postgres by starting the server and reading every route. Residual: these routes are reads only — the console can list and open but not author (create a role, draft a proposal); the propose/discovery stages remain inert by design. Platform 85 files / 2,081 tests, console 105 files / 1,389 tests, both typechecks + lint clean, no AI attribution, no existing test weakened.
...
### T-12  [RESOLVED via CLI (wave 9); no HTTP route / preview / PDF-DOCX residual]
...
**Resolution (wave 9, commit "pv documents — the document generator is reachable…").** `buildPlatform` composes the `DocumentStore` (Pg vs memory), the `TemplateRegistry`, and the `DocumentGenerator` — handed the platform's **one** already-composed `contactGate` (wave 6), not a second one, so an owner-facing document is gated by the same consent the ledger records — and registers `DOCUMENT_ACTIONS` in the one chokepoint. `pv documents` adds template register/approve/list/show, propose, and generate. Driven against Postgres by two independent parties and by hand: a draft refuses generation (`config.missing`); self-approval refuses (`approval.self_approval`); a different reviewer approves; an internal document is generated with full provenance (its content-derived data digest reproduces across independent runs); a consumer template requires two distinct approvers; an owner-facing generation with no consent is refused at the contact gate (`contact.no_consent`) and produces nothing; with consent recorded and a proposal-digest-bound approval granted by a different person, it is produced, recording an approver distinct from the producer, with the contact-evidence digest. Every refusal leaves zero document rows and the audit chain intact. **Residual, stated plainly:** no HTTP route and no console surface; the approval screen's artifact preview is still unwired; output is text/HTML only — PDF/DOCX remain a declared seam pending MVW's real formats (L7).
